...
def write_signal_output(args, output_mat, name, output_str):
    ...
    trial_str = append_jobid_to_string(args, output_str)
    #filename = os.path.join(output_dir, name + trial_str + '.csv')
    filename = os.path.join(output_dir,  str(args.sid) + '.npy')
    print(filename)
    np.save(filename, output_mat)
        #print(filename)
        #with open(filename, 'w') as csvfile:
            #csvwriter = csv.writer(csvfile)
            #csvwriter.writerows(output_mat)

def extract_signal(args, select_files, labels, datum):
...
def process_sig_electrodes(args, datum):
    ...
    extract_signal(args, datum, elec_signal,
...
def main():
    ...
    if args.sig_elec_file:
        ...
        extract_signal(args, select_files, labels, datum)

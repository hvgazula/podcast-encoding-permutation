...
def main(venn_key, run_num):
    shuffle_folder_name = '20210326-' + venn_key + str(run_num)
    base_folder_name = '20210326-' + venn_key
    file_prefix = '20210326-' + venn_key + '-'

    subjects = sorted(
        glob.glob(
            '/scratch/gpfs/hgazula/podcast-encoding/results/' + shuffle_folder_name + '/*'))
    ...
    for subject in subjects:
        ...
        main_elec_file_list = sorted(
            glob.glob(
                os.path.join(
                    '/scratch/gpfs/hgazula/podcast-encoding/results/', base_folder_name,
                    os.path.basename(subject), '*.csv')))
    ...
    df1.to_csv(file_prefix + 'pre_fdr.csv',
              index=False,
    ...
    df.to_csv(file_prefix + 'post_fdr.csv', index=False, columns=['subject', 'electrode'])
    ...
    # df2.to_csv(file_prefix + 'post_fdr_lh.csv', index=False, columns=['subject', 'electrode'])

# phase-1000-sig-elec-glove50d-perElec-FDR-01-LH-hg

if __name__ == "__main__":
    for venn_key in ['intersection', 'notinbobbi', 'notinharsha']:
        print(venn_key)
        for i in range(1, 6):
            main(venn_key, i)